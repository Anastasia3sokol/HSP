...
G$EnsemblId = gsub(";(.*)",'',G$EnsemblId)#официальная версия с аннотацией после точки
G$EnsemblId = gsub("\\.(.*)",'',G$EnsemblId)#неофициальная без аннотациии
length(unique(G$EnsemblId))  # 57 992 у меня 58243
